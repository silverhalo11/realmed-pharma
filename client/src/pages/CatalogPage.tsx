...
const BASE_SLIDES = 90;
const API_BASE = (import.meta.env.VITE_API_URL ?? '').replace(/\/$/, '');

const catalogSlides = Array.from({ length: BASE_SLIDES }, (_, i) => {
...
const CatalogPage = () => {
  ...
  const uploadedSlides = products
    ...
    .map((p) => ({
      src: /^https?:\/\//i.test(p.catalogImage!) ? p.catalogImage! : `${API_BASE}${p.catalogImage!.startsWith('/') ? p.catalogImage : `/${p.catalogImage}`}`,
      productName: p.name,
    }));

  const allSlides = [
    ...catalogSlides.map((src) => ({ src, productName: '' })),
    ...uploadedSlides,
  ];
    ...
    >
      ...
      <div style={{ flex: 1, position: 'relative' }}>
        <ZoomableImage
          ...
          alt={allSlides[current]?.productName || `Slide ${current + 1}`}
          onSwipeLeft={goNext}
          onSwipeRight={goPrev}
          fitMode="cover"
        />
